...
function recommendationColor(r:string){if(r==='within_appetite')return D.green;if(r==='outside_appetite')return D.red;return D.amber}
function badge(status:string){const s=String(status||'').toUpperCase();const c=s==='PROCESSED'?D.green:s==='REVIEW_REQUIRED'?D.amber:s==='PROCESSING'?D.teal:D.sub;return <span style={{fontSize:10,fontWeight:700,padding:'3px 8px',borderRadius:10,color:c,background:`${c}18`,border:`1px solid ${c}44`}}>{s||'UNKNOWN'}</span>}
function recBadge(r?:string|null){if(!r)return <span style={{fontSize:10,color:D.sub}}>—</span>;const c=recommendationColor(r);return <span style={{fontSize:10,fontWeight:700,padding:'3px 8px',borderRadius:10,color:c,background:`${c}18`,border:`1px solid ${c}44`}}>{r.replace(/_/g,' ').toUpperCase()}</span>}

type View='list'|'upload'|'trace'
export default function UwPage(){
 ...
 async function load(){setLoading(true);setError('');try{setJobs(await pasApi.uwJobs())}catch(e){setError(messageOf(e,'UW queue could not be loaded.'))}finally{setLoading(false)}}
 useEffect(()=>{load()},[])

 async function openTrace(id:string){setSelectedId(id);setView('trace');setTrace(null);try{setTrace(await pasApi.uwTrace(id))}catch(e){setError(messageOf(e,'Trace could not be loaded.'))}}
 ...
 return <div style={{maxWidth:1200,margin:'0 auto'}}>
  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:20}}>
   <div>
    ...
    <h2 style={{fontSize:20,color:D.text,margin:0}}>{view==='trace'?'UW Trace':'Underwriting Intake'}</h2>
    <p style={{fontSize:12,color:D.sub,margin:'4px 0 0'}}>Submission document intake, KB-grounded appetite assessment, and HubSpot deal creation</p>
   </div>
   {view==='list'&&<button style={button(D.orange)} onClick={()=>setView('upload')}>Upload submission</button>}
  </div>
  ...
  {view==='upload'&&<UwUploadForm onDone={()=>{setView('list');load()}}/>}

  {view==='trace'&&selectedId&&<UwTraceView trace={trace} jobId={selectedId}/>}
 </div>
}

function UwUploadForm({onDone}:{onDone:()=>void}){
 ...
 async function submit(e:React.FormEvent){
  ...
  try{
   ...
   const res=await pasApi.uwUpload(form)
   setResult(res)
  }catch(e){setError(messageOf(e,'UW upload failed.'))}finally{setBusy(false)}
 }
 ...
 return <form onSubmit={submit} style={{maxWidth:520}}><Section title="Submission document">
  ...
  <button style={button(D.orange)} disabled={busy}>{busy?'Uploading…':'Upload'}</button>
 </Section></form>
}

function UwTraceView({trace,jobId}:{trace:any;jobId:string}){
 useSetActiveRecord('uw',jobId)
 const viewerRef=useRef<DocumentViewerHandle>(null)
 const fetchBlob=useCallback(()=>pasApi.uwDocument(jobId),[jobId])
 ...
 const cited=trace.analysis_json?._cited_field_count
 const scored=trace.analysis_json?._scored_field_count
 const fields=trace.analysis_json?Object.fromEntries(Object.entries(trace.analysis_json).filter(([k])=>!k.startsWith('_')&&k!=='kb_findings')):null
 ...
 return <div style={{display:'grid',gridTemplateColumns:'minmax(0,1fr) minmax(320px,420px)',gap:14,alignItems:'start'}}>
  ...
  <div style={{display:'grid',gap:14}}>
   <Section title="Job">
    <div style={{fontSize:13,color:D.text}}>Job ID: {jobId}</div>
   ...
   <Section title="Analysis">
    <CitedFieldTree data={fields} onJump={jump}/>
   </Section>
   {trace.analysis_json?.kb_findings&&<Section title="KB-grounded appetite findings">
    <pre style={{fontSize:11,color:D.text,margin:0,whiteSpace:'pre-wrap',wordBreak:'break-word'}}>{JSON.stringify(trace.analysis_json.kb_findings,null,2)}</pre>
   </Section>}
  </div>
 </div>
}
